...
namespace nvhttp
{
    ...
    {
    public:
        SunshineHTTPS( boost::asio::io_context& io_context, boost::asio::ssl::context& ctx ) : SimpleWeb::HTTPS( io_context, ctx ) {}
    ...
    public:
        SunshineHTTPSServer( const std::string& certification_file, const std::string& private_key_file )
            : ServerBase< SunshineHTTPS >::ServerBase( 443 ), context( boost::asio::ssl::context::tls_server )
        {
    ...
    template < class T > void print_req( std::shared_ptr< typename SimpleWeb::ServerBase< T >::Request > request )
    {
    ...
    template < class T > void not_found( std::shared_ptr< typename SimpleWeb::ServerBase< T >::Response > response, std::shared_ptr< typename SimpleWeb::ServerBase< T >::Request > request )
    {
        print_req< T >( request );
    ...
    template < class T > void serverinfo( std::shared_ptr< typename SimpleWeb::ServerBase< T >::Response > response, std::shared_ptr< typename SimpleWeb::ServerBase< T >::Request > request )
    {
        print_req< T >( request );
    ...
    void launch( bool& host_audio, resp_https_t response, req_https_t request )
    {
        ...
        auto args = request->parse_query_string();
        if ( args.find( "rikey"s ) == std::end( args ) || args.find( "rikeyid"s ) == std::end( args ) || args.find( "localAudioPlayMode"s ) == std::end( args )
        ...
        tree.put( "root.sessionUrl0",
                  launch_session->rtsp_url_scheme + net::addr_to_url_escaped_string( request->local_endpoint().address() ) + ':' + std::to_string( net::map_port( rtsp_stream::RTSP_SETUP_PORT ) ) );
    ...
    void resume( bool& host_audio, resp_https_t response, req_https_t request )
    {
        ...
        auto args = request->parse_query_string();
        if ( args.find( "rikey"s ) == std::end( args ) || args.find( "rikeyid"s ) == std::end( args ) )
        {
            tree.put( "root.resume", 0 );
            tree.put( "root.<xmlattr>.status_code", 400 );
    ...
    void sleep( resp_https_t response, req_https_t request )
    {
        print_req< SunshineHTTPS >( request );
    ...
    void execSuperCmd( resp_https_t response, req_https_t request )
    {
        print_req< SunshineHTTPS >( request );
    ...
    template < class T > void shutdown( std::shared_ptr< typename SimpleWeb::ServerBase< T >::Response > response, std::shared_ptr< typename SimpleWeb::ServerBase< T >::Request > request )
    {
        print_req< T >( request );
        ...
        BOOL result = ExitWindowsEx( EWX_SHUTDOWN | EWX_FORCE | EWX_POWEROFF, SHTDN_REASON_MAJOR_OTHER | SHTDN_REASON_FLAG_PLANNED );

        // 如果失败，尝试第二种方式
        if ( !result )
        {
            result = InitiateSystemShutdownEx( nullptr, nullptr, 0, TRUE, FALSE, SHTDN_REASON_MAJOR_OTHER | SHTDN_REASON_FLAG_PLANNED );
        }

        pt::ptree tree;
        if ( result )
        {
            BOOST_LOG( info ) << "Shutdown command issued successfully";
            tree.put( "root.shutdown", 1 );
        }
        else
        {
            DWORD err = GetLastError();
            BOOST_LOG( error ) << "Failed to issue shutdown command: " << err;
            tree.put( "root.shutdown", 0 );
            tree.put( "root.error", err );
        }
    ...
    void start()
    {
        ...
        https_server.resource[ "^/shutdown$" ][ "GET" ]   = shutdown< SunshineHTTPS >;
        ...
        http_server.resource[ "^/shutdown$" ][ "GET" ]   = shutdown< SimpleWeb::HTTP >;
