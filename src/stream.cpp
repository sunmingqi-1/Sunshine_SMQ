...
#pragma pack( push, 1 )
    ...
    static_assert( sizeof( video_short_frame_header_t ) == 8, "Short frame header must be 8 bytes" );
...
#pragma pack( pop )
    ...
    struct broadcast_ctx_t
    {
        ...
        mic_in::ctx_t    mic_ctx;  // Microphone input context
    ...
    {
        static_assert( max_payload_size >= sizeof( control_encrypted_t ) + sizeof( crypto::cipher::tag_size ), "max_payload_size >= sizeof(control_encrypted_t) + sizeof(crypto::cipher::tag_size)" );
    ...
    void control_server_t::call( std::uint16_t type, session_t* session, const std::string_view& payload, bool reinjected )
    {
        ...
        {
            BOOST_LOG( debug ) << "type [Unknown] { "sv << util::hex( type ).to_string_view() << " }"sv << std::endl
                               << "---data---"sv << std::endl
    ...
    {
        server->map( packetTypes[ IDX_PERIODIC_PING ], []( session_t* session, const std::string_view& payload ) { BOOST_LOG( verbose ) << "type [IDX_PERIODIC_PING]"sv; } );

        server->map( packetTypes[ IDX_START_A ], [ & ]( session_t* session, const std::string_view& payload ) { BOOST_LOG( debug ) << "type [IDX_START_A]"sv; } );

        server->map( packetTypes[ IDX_START_B ], [ & ]( session_t* session, const std::string_view& payload ) { BOOST_LOG( debug ) << "type [IDX_START_B]"sv; } );
        ...
        server->map( packetTypes[ IDX_LOSS_STATS ], [ & ]( session_t* session, const std::string_view& payload ) {
            ...
            BOOST_LOG( verbose ) << "type [IDX_LOSS_STATS]"sv << std::endl
        ...
        server->map( packetTypes[ IDX_INVALIDATE_REF_FRAMES ], [ & ]( session_t* session, const std::string_view& payload ) {
            ...
            BOOST_LOG( debug ) << "type [IDX_INVALIDATE_REF_FRAMES]"sv << std::endl << "firstFrame [" << firstFrame << ']' << std::endl << "lastFrame [" << lastFrame << ']';
    ...
    void recvThread( broadcast_ctx_t& ctx )
    {
        ...
        auto recv_func_init = [ & ]( udp::socket& sock, int buf_elem, std::map< av_session_id_t, message_queue_t >& peer_to_session ) {
            recv_func[ buf_elem ] = [ &, buf_elem ]( const boost::system::error_code& ec, size_t bytes ) {
                auto fg = util::fail_guard( [ & ]() { sock.async_receive_from( asio::buffer( buf[ buf_elem ] ), peer, 0, recv_func[ buf_elem ] ); } );
    ...
    void videoBroadcastThread( udp::socket& sock )
    {
        ...
        while ( auto packet = packets->pop() )
        {
            ...
            frame_header.frameType                  = packet->is_idr() ? 2 : packet->after_ref_frame_invalidation ? 5 : 1;
            ...
            decltype( fec_blocks )::iterator               fec_blocks_begin = std::begin( fec_blocks ), fec_blocks_end = std::begin( fec_blocks ) + fec_blocks_needed;
            ...
            try
            {
                ...
                std::for_each( fec_blocks_begin, fec_blocks_end, [ & ]( std::string_view& current_payload ) {
                    ...
                    auto batch_info   = platf::batched_send_info_t{
                        shards.headers.begin(), shards.prefixsize,          shards.payload_buffers, shards.blocksize, 0, 0, ( uintptr_t )sock.native_handle(),
                        peer_address,           session->video.peer.port(), session->localAddress,
                    };
                    ...
                    for ( auto x = 0; x < shards.size(); ++x )
                    {
                        ...
                        if ( x - next_shard_to_send + 1 >= send_batch_size || x + 1 == shards.size() )
                        {
                            ...
                            {
                                auto due = ratecontrol_frame_start + std::chrono::duration_cast< std::chrono::nanoseconds >( 1ms ) * ratecontrol_frame_packets_sent / ratecontrol_packets_in_1ms;
                    ...
                    ratecontrol_next_frame_start =
                        ratecontrol_frame_start + std::chrono::duration_cast< std::chrono::nanoseconds >( 1ms ) * ratecontrol_frame_packets_sent / ratecontrol_packets_in_1ms;

                    frame_network_latency_logger.second_point_now_and_log();

                    BOOST_LOG( verbose ) << "Sent Frame seq ["sv << packet->frame_index() << "] pts ["sv << timestamp << "] shards ["sv << shards.size() << "/"sv << shards.percentage << "%]"sv
                                         << ( frame_is_dupe ? " Dupe" : "" ) << ( packet->is_idr() ? " Key" : "" ) << ( packet->after_ref_frame_invalidation ? " RFI" : "" );
            ...
            try
            {
                ...
                if ( ( sequenceNumber + 1 ) % RTPA_DATA_SHARDS == 0 )
                {
                    ...
                    for ( auto x = 0; x < RTPA_FEC_SHARDS; ++x )
                    {
                        ...
                        auto send_info = platf::send_info_t{
                            ( const char* )&fec_packet, sizeof( fec_packet ),  ( const char* )shards_p[ RTPA_DATA_SHARDS + x ], ( size_t )bytes, ( uintptr_t )sock.native_handle(), peer_address,
                            session->audio.peer.port(), session->localAddress,
                        };
                        platf::send( send_info );
    ...
    int start_broadcast( broadcast_ctx_t& ctx )
    {
        ...
        auto micro_port     = net::map_port( MICRO_STREAM_PORT );  //  // MIC-IN
        ...
        // ── MIC-IN: 启动 UDP→Opus→VB-Cable 线程 ──────────────────────
        if ( mic_in::start( ctx.mic_ctx, micro_port, "CABLE Input" ) != 0 )
        {
            BOOST_LOG( error ) << "MIC-IN: failed to start on UDP port " << micro_port;
        }
        else
        {
            BOOST_LOG( info ) << "MIC-IN: listening on UDP " << micro_port;
        }
        // ─────────────────────────────────────────────────────────────
        return 0;
    }
    ...
    void end_broadcast( broadcast_ctx_t& ctx )
    {
        ...
        audio_packets->stop();
        // ── MIC-IN: 请求停止线程 ─────────────────────────────────────
        mic_in::stop( ctx.mic_ctx );  // 会 join 内部线程
        // ─────────────────────────────────────────────────────────────
    ...
    {
        auto fg = util::fail_guard( [ & ]() { session::stop( *session ); } );
    ...
    namespace session
    {
        ...
        std::shared_ptr< session_t > alloc( config_t& config, rtsp_stream::launch_session_t& launch_session )
        {
            ...
            session->control.cipher              = crypto::cipher::gcm_t{ launch_session.gcm_key, false };
            ...
            if ( config.encryptionFlagsEnabled & SS_ENC_VIDEO )
            {
                ...
                session->video.cipher         = crypto::cipher::gcm_t{ launch_session.gcm_key, false };
