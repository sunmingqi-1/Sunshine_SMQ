...
namespace confighttp
{
    ...
    void send_unauthorized( resp_https_t response, req_https_t request )
    {
        ...
        const SimpleWeb::CaseInsensitiveMultimap headers{ { "WWW-Authenticate", R"(Basic realm="Sunshine Gamestream Host", charset="UTF-8")" } };
    ...
    void send_redirect( resp_https_t response, req_https_t request, const char* path )
    {
        ...
        const SimpleWeb::CaseInsensitiveMultimap headers{ { "Location", path } };
    ...
    bool authenticate( resp_https_t response, req_https_t request )
    {
        ...
        auto fg = util::fail_guard( [ & ]() { send_unauthorized( response, request ); } );
    ...
    void getHtmlPage( resp_https_t response, req_https_t request, const std::string& pageName, bool requireAuth = true )
    {
        if ( requireAuth && !authenticate( response, request ) )
            return;
     ...
     * 处理静态资源文件
     */
    void getStaticResource( resp_https_t response, req_https_t request, const std::string& path, const std::string& contentType )
    {
        print_req( request );
    ...
    bool saveVddSettings( std::string resArray, std::string fpsArray, std::string gpu_name )
    {
        ...
        auto idd_option_path = std::filesystem::path( systemDrive ) / "\\" / "VirtualDisplayDriver" / "vdd_settings.xml";
    ...
    void boom( resp_https_t response, req_https_t request )
    {
        if ( !authenticate( response, request ) )
            return;
    ...
    void start()
    {
        ...
        auto accept_and_run = [ & ]( auto* server ) {
            try
            {
                server->start( []( unsigned short port ) { BOOST_LOG( info ) << "Configuration UI available at [https://localhost:"sv << port << "]"; } );
            }
