...
#include <AMF/components/VideoEncoderVCE.h>
        ...
        template < class T > std::optional< int > quality_from_view( const std::string_view& quality_type, const std::optional< int >( &original ) )
        {
#define _CONVERT_( x )            \
...
#undef _CONVERT_
        ...
        template < class T > std::optional< int > rc_from_view( const std::string_view& rc, const std::optional< int >( &original ) )
        {
#define _CONVERT_( x )  \
    if ( rc == #x##sv ) \
    return ( int )T::x
...
#undef _CONVERT_
        ...
        template < class T > std::optional< int > usage_from_view( const std::string_view& usage, const std::optional< int >( &original ) )
        {
#define _CONVERT_( x )     \
    if ( usage == #x##sv ) \
    return ( int )T::x
...
#undef _CONVERT_
    ...
    template < class It > It skip_list( It skipper, It end )
    {
        int stack = 1;
    ...
    std::pair< std::string_view::const_iterator, std::optional< std::pair< std::string, std::string > > > parse_option( std::string_view::const_iterator begin, std::string_view::const_iterator end )
    {
        ...
        return std::make_pair( endl, std::make_pair( to_string( begin, end_name ), to_string( begin_val, endl ) ) );
    }
    ...
    template < typename T, typename F > void generic_f( std::unordered_map< std::string, std::string >& vars, const std::string& name, T& input, F&& f )
    {
        std::string tmp;
    ...
    template < class F > void int_f( std::unordered_map< std::string, std::string >& vars, const std::string& name, int& input, F&& f )
    {
        std::string tmp;
        string_f( vars, name, tmp );
        if ( !tmp.empty() )
    ...
    template < class F > void int_f( std::unordered_map< std::string, std::string >& vars, const std::string& name, std::optional< int >& input, F&& f )
    {
        std::string tmp;
        string_f( vars, name, tmp );
        if ( !tmp.empty() )
    ...
    bool to_bool( std::string& boolean )
    {
        ...
        return boolean == "true"sv || boolean == "yes"sv || boolean == "enable"sv || boolean == "enabled"sv || boolean == "on"sv
    ...
    void list_display_mode_remapping_f( std::unordered_map< std::string, std::string >& vars, const std::string& name, std::vector< video_t::display_mode_remapping_t >& input )
    {
        std::string string;
        string_f( vars, name, string );
        ...
        for ( auto& [ _, entry ] : jsonTree.get_child( "display_mode_remapping"s ) )
        {
            ...
            input.push_back( video_t::display_mode_remapping_t{ type.value_or( "" ), received_resolution.value_or( "" ), received_fps.value_or( "" ), final_resolution.value_or( "" ),
    ...
    void sync_idd_f( std::unordered_map< std::string, std::string >& vars, const std::string& name, std::string& input )
    {
        if ( input == "ZakoHDR" )
        {
