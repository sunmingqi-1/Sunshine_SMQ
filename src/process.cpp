...
namespace proc
{
    ...
    int proc_t::execute( int app_id, std::shared_ptr< rtsp_stream::launch_session_t > launch_session )
    {
        ...
        auto iter = std::find_if( _apps.begin(), _apps.end(), [ &app_id ]( const auto app ) { return app.id == std::to_string( app_id ); } );
        ...
        auto fg = util::fail_guard( [ & ]() { terminate(); } );
        ...
        for ( ; _app_prep_it != std::end( _app.prep_cmds ); ++_app_prep_it )
        {
            ...
            boost::filesystem::path working_dir = _app.working_dir.empty() ? find_working_directory( cmd.do_cmd, _env ) : boost::filesystem::path( _app.working_dir );
        ...
        {
            boost::filesystem::path working_dir = _app.working_dir.empty() ? find_working_directory( cmd, _env ) : boost::filesystem::path( _app.working_dir );
        ...
        {
            boost::filesystem::path working_dir = _app.working_dir.empty() ? find_working_directory( _app.cmd, _env ) : boost::filesystem::path( _app.working_dir );
    ...
    void proc_t::terminate()
    {
        ...
        for ( ; _app_prep_it != _app_prep_begin; --_app_prep_it )
        {
            ...
            boost::filesystem::path working_dir = _app.working_dir.empty() ? find_working_directory( cmd.undo_cmd, _env ) : boost::filesystem::path( _app.working_dir );
    ...
    void proc_t::run_menu_cmd( std::string cmd_id )
    {
        auto iter = std::find_if( _app.menu_cmds.begin(), _app.menu_cmds.end(), [ &cmd_id ]( const auto menu_cmd ) { return menu_cmd.id == cmd_id; } );
        ...
        while ( dollar != std::end( val_raw ) )
        {
            ...
            if ( next != std::end( val_raw ) )
            {
                ...
                case '(': {
                    ...
                    auto itr = std::find_if( env.cbegin(), env.cend(), [ & ]( const auto& e ) { return boost::iequals( e.get_name(), var_name ); } );
                    if ( itr != env.cend() )
                    {
            ...
            for ( auto& [ _, app_node ] : apps_node )
            {
                ...
                if ( !exclude_global_prep.value_or( false ) )
                {
                    ...
                    for ( auto& prep_cmd : config::sunshine.prep_cmds )
                    {
                        ...
                        prep_cmds.emplace_back( std::move( do_cmd ), std::move( undo_cmd ), std::move( prep_cmd.elevated ) );
                    }
                }
                ...
                if ( prep_nodes_opt )
                {
                    ...
                    for ( auto& [ _, prep_node ] : prep_nodes )
                    {
                        ...
                        prep_cmds.emplace_back( parse_env_val( this_env, do_cmd.value_or( "" ) ), parse_env_val( this_env, undo_cmd.value_or( "" ) ), std::move( elevated.value_or( false ) ) );
                    }
                }
